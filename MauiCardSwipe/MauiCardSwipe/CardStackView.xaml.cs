...
public partial class CardStackView : ContentView
{
    ...
    public static BindableProperty LoadNextItemsCommandProperty = BindableProperty.Create(nameof(LoadNextItemsCommand), typeof(ICommand), typeof(CardStackView), null);
    public ICommand LoadNextItemsCommand
    {
        get { return (ICommand)GetValue(LoadNextItemsCommandProperty); }
        set { SetValue(LoadNextItemsCommandProperty, value); }
    }
    ...
    public void Setup()
    {
        ...
        for (var i = numberOfCards - 1; i >= 0; i--)
        {
            ...
            var topCard = (CardView)CardStack.Children[0];
            var x = topCard.X;
        }
    ...
    void HandleTouchRunning(float xDiff)
    {
        ...
        var topCard = (CardView)CardStack.Children[numberOfCards - 1];
        var backCard = (CardView)CardStack.Children[numberOfCards - 2];
    ...
    async Task HandleTouchCompleted()
    {
        ...
        if (Math.Abs(cardDistance) >= CardMoveDistance)
        {
            ...
            await ((CardView)topCard).TranslateTo(cardDistance > 0 ? this.Width * 2 : -this.Width * 2, 0, defaultAnimationLength, Easing.SinIn);
            ((CardView)topCard).IsVisible = false;
        ...
        else
        {
            ...
            await Task.WhenAll(
               ...
               ((CardView)topCard).TranslateTo((-((CardView)topCard).X), -((CardView)topCard).Y, defaultAnimationLength, Easing.SpringOut),
                ((CardView)topCard).RotateTo(0, defaultAnimationLength, Easing.SpringOut),

                 // Scale the back card down
                 ((CardView)backCard).ScaleTo(defaultSubcardScale, defaultAnimationLength, Easing.SpringOut)
            );
        }
    ...
    void ShowNextCard()
    {
        ...
        if (ItemsSource?.Count - itemIndex == 0)
        {
            activityIndicator.IsRunning = true;
            if (LoadNextItemsCommand != null && LoadNextItemsCommand.CanExecute(null))
                LoadNextItemsCommand.Execute(null);
        }
        ...
        if (itemIndex != 0)
        {
            ...
            ((CardView)topCard).Scale = defaultSubcardScale;
